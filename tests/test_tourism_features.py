...
def test_recommendation_uses_only_safe_candidates_and_validates_ai(monkeypatch, caplog):
    ...
    assert "Choose five varied stops whenever five can fit" in captured["messages"][0]["content"]
...
def test_recommendation_randomly_samples_a_weighted_candidate_pool(monkeypatch):
    candidates = [
        SimpleNamespace(
            ...
            region="강원특별자치도",
            sigun="강릉시",
            sport_name="walking" if item_id == 1 else None,
...
def test_stamp_submission_requires_owned_published_mission_and_prefix():
    ...
    invalid = StampSubmissionCreate(
        passport_id=1, stamp_id=2, object_key="proofs/9/2/x.jpg"
    )
